...
// Live location sharing, authenticated as the logged-in user. A linked
// Guardian receives these updates in real time via Socket.io, or can poll
// GET /v1/location/current/:userId with their own guardian token.
export const locationApi = {
  start: (location?: { latitude: number; longitude: number; accuracy?: number }) =>
    request<{ success: boolean; message: string; data: { location: CurrentLocationDto } }>('/v1/location/start', {
      method: "POST" as HttpMethod,
      body: location ?? {},
    }),
  update: (latitude: number, longitude: number, accuracy?: number) =>
    request<{ success: boolean; message: string; data: { location: CurrentLocationDto } }>('/v1/location/update', {
      method: "POST" as HttpMethod,
      body: { latitude, longitude, accuracy },
    }),
  stop: () =>
    request<{ success: boolean; message: string }>('/v1/location/stop', {
      method: "POST" as HttpMethod,
    }),
};
